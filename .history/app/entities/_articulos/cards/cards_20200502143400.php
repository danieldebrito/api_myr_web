...
class cards {
    public static function readAll(){
        try {
            ...
            (
                "SELECT id_articulo, linea, marca, combustible, motor, cilindrada, competicion, producto, aplicacion, aplicacionEspecifica, urlImgPeq, materialDetalle, espesor, nueva, promo, stock, prioridadBusqueda,
                GROUP_CONCAT( DISTINCT modelo SEPARATOR ' - ' ) as 'modelo'
                FROM cards 
                WHERE 1 group by id_articulo"
        );
            $consulta->execute();
    ...
    public function readParams($linea, $marca, $combustible, $motor, $modelo, $cilindrada, $competicion, $producto, $aplicacion){
        ...
        $instruccionSQL = "SELECT id_articulo, linea, marca, combustible, motor, cilindrada, competicion, producto, aplicacion, aplicacionEspecifica, urlImgPeq, materialDetalle, espesor, nueva, promo, stock, prioridadBusqueda,
        GROUP_CONCAT( DISTINCT modelo SEPARATOR ' - ' ) as 'modelo'
        FROM cards 
        WHERE 1";
    ...
    public static function readById ($id_articulo){
        ...
        GROUP_CONCAT( DISTINCT modelo SEPARATOR ' - ' ) as 'modelo'
        FROM cards 
        WHERE `id_articulo` LIKE "."'%"."$id_articulo"."%'".' group by id_articulo'
        );
    ...
    public static function buscarPorFrase ($frase){
       ...
       WHERE 1";
        ...
        foreach ($fraseArray  as &$item ) {
            ...
            LIKE "."'"."%".$item.'%'."'".' group by id_articulo';
        } try {
            $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
            $consulta = $objetoAccesoDato->RetornarConsulta
            ("$instruccionSQL");
